...
# Usage: ut-run [gl | git] [task-number]
...
if [ $1 == "git" ]
then
  c git clone https://github.com/helenaandrade/fit-cli
  c cd fit-cli
  c git branch
  c git log --oneline
else
  c mkdir fit-cli
  c cd fit-cli
  c gl init https://github.com/helenaandrade/fit-cli
  c gl branch
  c gl history -c
fi
c chmod -R ugo+rw ~/Desktop/fit-cli
c pwd
c ls
